...
from src.core import Graph, Timeline, WorldMap, CharacterCard
...
class KnowledgeBaseProtocol(Protocol):
    ...
    async def add_content(self, content: str, content_type: str = "", metadata: dict | None = None) -> None:
        ...
...
class BaseAgent:
    ...
    async def _retrieve_knowledge(self, query: str, top_k: int = 5) -> list[dict]:
        ...
        if hasattr(self._context, 'knowledge') and self._context.knowledge:
            return await self._context.knowledge.retrieve(query, top_k)
