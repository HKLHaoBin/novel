...
def build_full_context(context: Any) -> str:
    ...
    if context.graph:
        ...
        if world_nodes:
            ...
            for node in world_nodes:
                ...
                if rules:
                    parts.append("├──规则体系")
                    for i, rule in enumerate(rules if isinstance(rules, list) else [rules]):
                ...
                if forces:
                    parts.append("├──主要势力")
                    for i, force in enumerate(forces if isinstance(forces, list) else [forces]):
    ...
    if context.graph:
        ...
        if main_plots or sub_plots:
            ...
            if main_plots:
                parts.append("\n主线")
                for i, node in enumerate(main_plots):
                    ...
                    if stages:
                        parts.append("└──阶段")
                        for j, stage in enumerate(stages if isinstance(stages, list) else [stages]):
                            marker = "   └──" if j == len(stages) - 1 else "   ├──"
                            parts.append(f"{marker}{stage}")
            
            if sub_plots:
                parts.append("\n支线")
                for i, node in enumerate(sub_plots):
                    name = node.attrs.get('name', node.id)
                    parts.append(f"├──{name}")
        ...
        for char_id, char in context.characters.items():
            ...
            if public_abilities:
                ...
                for i, ab in enumerate(public_abilities):
                    marker = "│  └──" if i == len(public_abilities) - 1 and not hidden_abilities and not future_abilities else "│  ├──"
                    parts.append(f"{marker}{ab.name}: {ab.description[:30]}..." if len(ab.description) > 30 else f"{marker}{ab.name}: {ab.description}")
            
            if hidden_abilities:
                parts.append("├──隐藏能力（读者未知）")
                for i, ab in enumerate(hidden_abilities):
                    marker = "│  └──" if i == len(hidden_abilities) - 1 and not future_abilities else "│  ├──"
            ...
            if char.notes:
                ...
                for note in char.notes[-3:]:  # 最近3条
                    parts.append(f"   └──{note[:50]}..." if len(note) > 50 else f"   └──{note}")
            else:
                parts.append("└──备注: 无")
        ...
        if points:
            ...
            for i, p in enumerate(points):
                ...
                if p.character_locations:
                    for j, (char_id, loc_id) in enumerate(p.character_locations.items()):
    ...
    if context.world_map and context.world_map.locations:
        ...
        # 找出顶级地点（无父级）
        root_locs = [loc for loc in context.world_map.locations.values() if not loc.parent_id]
