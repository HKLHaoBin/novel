...
class Auditor(BaseAgent):
    ...
    async def execute(self, context: AgentContext) -> AgentResult:
        ...
        Args:
            context: 执行上下文，需要包含:
                - user_input: 待审计的内容
                - extra.get("audit_type"): 审计类型 (full/quick/time/space/character/plot/world)
        ...
        try:
            if audit_type == "full":
                ...
                issues = await self._single_dimension_audit(context, content, audit_type)
            
            suggestions = self._generate_suggestions(issues)
            
            return AgentResult(
                success=len(issues) == 0 or all(i.get("severity") != "severe" for i in issues),
...
async def quick_check(context: AgentContext, content: str) -> list[dict]:
    ...
    result = await auditor.execute(AgentContext(
        graph=context.graph,
