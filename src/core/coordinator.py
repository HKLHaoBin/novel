...
from src.core.graph import Edge, EdgeType, Graph, Node, NodeType, Timeline, TimePoint
...
class NovelCoordinator:
    ...
    def _restore_from_snapshot(self, context: NovelContext) -> None:
        ...
        for _nid, n in graph_data.get("nodes", {}).items():
            node = Node(
                id=n["id"],
        ...
        for _eid, e in graph_data.get("edges", {}).items():
        ...
        for _pid, p in timeline_data.get("points", {}).items():
            point = TimePoint(
                id=p["id"],
                label=p["label"],
        ...
        for _lid, loc_data in map_data.get("locations", {}).items():
        ...
        for _eid, e in map_data.get("edges", {}).items():
            from src.core.map import LocationEdge

            context.world_map.add_edge(
                LocationEdge(
                    id=e["id"],
        ...
        for cid, c in snapshot.characters_data.items():
            ...
            for _aid, a in c.get("abilities", {}).items():
                card.add_ability(
                    Ability(
                        name=a["name"],
    ...
    def complete_chapter(
        self, context: NovelContext, chapter_num: int, content: str, title: str = ""
    ) -> None:
        """完成章节"""
