...
class NovelStateManager:
    ...
    def save_chapter(
        self, title: str, chapter_num: int, content: str, chapter_title: str = ""
    ) -> str:
        """保存成品章节
    ...
    def load_draft(
        self, title: str, filename: str | None = None
    ) -> NovelSnapshot | None:
        """加载半成品状态"""
    ...
    def _deserialize_snapshot(self, data: dict) -> NovelSnapshot:
        ...
        progress = GenerationProgress(
            current_phase=GenerationPhase(
                progress_data.get("current_phase", "not_started")
            ),
    ...
    def complete_chapter(
        self, snapshot: NovelSnapshot, chapter_num: int, content: str, title: str = ""
    ) -> None:
        """完成章节"""
