...
class KnowledgeBase:
    ...
    def __init__(
        ...
        if not MEMORY_AVAILABLE:
            raise ImportError("superficial-thinking 库未安装，请确保 dawn_shuttle_superficial_thinking 可用")
    ...
    async def initialize(self, llm_provider, system_prompt: str = "") -> "KnowledgeBase":
        """
        初始化知识库
...
"""
    ...
    async def retrieve(
        ...
        for mem in result.fuzzy_memories[:top_k]:
            memories.append({
                "type": "fuzzy",
                "summary": mem.summary,
                "keywords": mem.keywords.primary if hasattr(mem, 'keywords') else [],
    ...
    async def query(self, question: str) -> str:
        ...
        return await self._memory.query_memory(question)
    ...
    def get_context(self, include_fuzzy: bool = True) -> list[dict]:
        ...
        return self._memory.get_context(include_fuzzy=include_fuzzy)
